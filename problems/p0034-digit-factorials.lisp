(defpackage #:project-euler/problems/p0034-digit-factorials
    (:nicknames #:p0034)
    (:use #:cl #:project-euler/commons)
    (:export #:solve))
(in-package #:project-euler/problems/p0034-digit-factorials)
...
(defun main-original ()
    ...
    (do ((i 10 (+ i 1))) ((= i limit) s)
        (if (= i (reduce #'+ (map 'list #'factorial (digits i))))
            (setf s (+ s i))))))

(defun main () (declare (optimize (speed 3) (safety 0) (debug 0)))

  (let ((factorials (make-array 10
                        :element-type 'fixnum
                        :initial-contents (loop for i of-type fixnum from 0 to 9 collect (factorial i))))
        (limit 2540160)
        (total-sum 0))
    ...
    (loop for i of-type fixnum from 10 below limit
        do (let ((sum 0)  (temp i)) (declare (type fixnum sum temp))
            ...
            (loop while (plusp temp) do (multiple-value-bind (q r) (truncate temp 10)
