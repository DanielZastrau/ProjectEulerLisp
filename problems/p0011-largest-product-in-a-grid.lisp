(defpackage #:project-euler/problems/p0011-largest-product-in-a-grid
    (:nicknames #:p0011)
    (:use #:cl #:project-euler/commons)
    (:export #:solve))
(in-package #:project-euler/problems/p0011-largest-product-in-a-grid)
...
(defmacro grid-product (row col trow tcol)
    "trow 1 tcol 0 -> vertical movement
    trow 0 tcol 1 -> horizontal movement
    trow 1 tcol 1 -> diagonal movement"
    `(* ,@(loop for i from 0 to 3
        collect (let ((r (if (zerop trow) row `(+ ,row ,(* i trow))))
                      (c (if (zerop tcol) col `(+ ,col ,(* i tcol)))))
                    `(nth ,c (nth ,r *grid*))))))
(defmacro update-mx (condition row col trow tcol)
    `(when ,condition
        (let ((product (grid-product ,row ,col ,trow ,tcol)))
        (when (> product mx) (setf mx product)))))

(defun main ()
    (let ((mx 0))
    (do ((row 0 (1+ row))) ((= row 20) mx)
        (do ((col 0 (1+ col))) ((= col 20))
            (update-mx (< col 17) row col 0 1)
            (update-mx (< row 17) row col 1 0)
            (update-mx (and (< col 17) (< row 17)) row col 1 1)
            (update-mx (and (> col 2) (< row 17)) row col 1 -1)))))
